...
TckJS.Util = TckJS.prototype = {
	...
	, Alert : function(msg,callbackOk){
		if ( this.AlertMode == "layer" ) 
		{
			...
			/* 2020-09-22 HSH 팝업창 엔터 또는 스페이스 키로 닫기 */
			$(window).keyup(function (){
				if(event.keyCode == 32 || event.keyCode == 13){
					if($(".tck-layer").length != 0) $(".btn_yes").click();
				}
			})
			/* 2020-09-22 HSH 팝업창 엔터 또는 스페이스 키로 닫기 끝 */
...
TckJS.PageUtil = TckJS.prototype = {
	...
	init : function(setData,callback) {
		...
		if($('#searchDateFrom[type=text]').length == 1){
			...
			$("#searchDateFrom").val(Common.GetDate(new Date(new Date().setDate(1))));
		}
		...
		if(setData) {
			for(var i = 0 ; i < setData.length; i++) {
				if($("#" + setData[i].formID).find("#" + setData[i].targatID).length == 1) {
					...
					if(el.attr("id") == "pageNo"){
						if(setData[i].val == "") setData[i].val = "0";
						el.val(setData[i].val);
						continue;
					}
		...
		/* 2020-09-22 HSH 키워드 검색 공백 제거 */
		$("#searchKeyword").blur(function(){
			this.value = this.value.trim();
		})
		/* 2020-09-22 HSH 키워드 검색 공백 제거 끝 */
	},
	...
	setDatePicker : function(input_Date) {
		var ToDate = new Date();
		...
		if(input_Date == "W"){
			...
			if(Month == 0){
				...
				FromDate = new Date(FromDate.setMonth(11));
				...
				ToDate = new Date(new Date().setMonth(new Date().getMonth()));
				ToDate = new Date(ToDate.setDate(0));
		...
		else if(input_Date == "Y"){
			FromDate = new Date(new Date().setFullYear(new Date().getFullYear() - 1));
		}

		
		if($('#searchDateFrom[type=text]').length == 1){
			$("#searchDateFrom").val(Common.GetDate(FromDate));
		}
		
		if($('#searchDateTo[type=text]').length == 1){
			$("#searchDateTo").val(Common.GetDate(ToDate));
		}
		
	},
	goSearch : function () {
		if($("#Searchfrm").length == 1 && $("#Searchfrm").find("#pageNo").length == 1){
			$("#Searchfrm").find("#pageNo").val(0);
		}

		if($("#searchConType").length == 1){
			if($("#searchConType").val() == "cpId"){
				$("#searchKeyword").val($("#searchKeyword_select").val());
			}else {
				if($("#searchKeyword_input").length == 1){
					$("#searchKeyword").val($("#searchKeyword_input").val());
				}
			}
		}
