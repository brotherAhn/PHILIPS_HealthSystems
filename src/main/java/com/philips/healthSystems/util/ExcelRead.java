    ...
    public List<List<Object>> readExcelToListXlS(MultipartFile multipartFile)
        ...
        for (int rowIndex = 0; rowIndex < curSheet.getPhysicalNumberOfRows(); rowIndex++) {
            ...
            if (rowIndex != 0) {
                ...
                int cells = curRow == null ? 0 : curRow.getPhysicalNumberOfCells();
                ...
                for (int cellnum = 0; cellnum < cells; cellnum++) {
                    ...
                    if (cell != null) {
                        if (cell.getCellTypeEnum() != CellType.STRING) cell.setCellType(CellType.STRING);
                        if (!"".equals(cell.getStringCellValue())) {
                            // cell 탐색 for문
                            for (int cellIndex = 0; cellIndex < curRow.getPhysicalNumberOfCells(); cellIndex++) {
                                curCell = curRow.getCell(cellIndex);
                                if (curCell.getCellTypeEnum() != CellType.STRING) curCell.setCellType(CellType.STRING);
